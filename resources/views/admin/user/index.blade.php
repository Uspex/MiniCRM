        ...
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between g-3">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">{{ __('user.title') }}</h3>
                </div>
                <div class="nk-block-head-content">
                    <div class="nk-block-head-content">
                        <a href="{{ route('admin.user.create') }}" class="btn btn-primary"><em class="icon ni ni-plus"></em><span>{{ __('user.add') }}</span></a>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <form method="GET" action="{{ route(request()->route()->getName(), [request()->get('page')]) }}">
                                <div class="row gx-6 gy-3">
                                    <div class="col-12 col-sm-6 col-md-3">
                                        <input class="form-control" placeholder="{{ __('user.search.name') }}" name="name" type="text" value="{{ request()->get('name') }}">
                                    </div>
                                    <div class="col-12 col-sm-6 col-md-3">
                                        <input class="form-control" placeholder="{{ __('user.search.username') }}" name="username" type="text" value="{{ request()->get('username') }}">
                                    </div>
                                    <div class="col-12 col-sm-6 col-md-3">
                                        <input class="form-control" placeholder="{{ __('user.search.email') }}" name="email" type="text" value="{{ request()->get('email') }}">
                                    </div>
                                    <div class="col-12 col-sm-6 col-md-3">
                                                ...
                                                <div class="drodown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        <ul class="link-list-opt no-bdr">
                                                            <li><a href="{{ route('admin.user.edit', $item->id) }}"><em class="icon ni ni-edit"></em><span>{{ __('common.item_edit') }}</span></a></li>
                                                            <li>
