...
!function (NioApp, $) {
  ...
  NioApp.Select2.init = function () {
    ...
    function formatOptionWithClass(state) {
      if (!state.id || !state.element) return state.text;
      var className = $(state.element).attr('class');
      if (className) {
    ...
    NioApp.Select2('.js-select2', {
          placeholder: 'Select an option', // Customize your placeholder text
          // allowClear: true                // This is the key to enable clearing
          templateResult: formatOptionWithClass,
          templateSelection: formatOptionWithClass
      });
  };
